...
Mo = 0               // É o momento excitante aplicado ao sistema

//Geração do vetor de instantes de tempo para o integrador
t0 = 0.0;            // Instante de tempo inicial
tf = 10.0;           // Instante de tempo final
dt = 0.01;           // Intervalo de tempo para obter 100 fps (intervalo de 1ms entre frames)
...
realtimeinit(0.025); // Tempo entre um frame e outro

//Loop da animação (atualiza-se as coordenadas para cada imagem)
i = 2;
while (i <= length(t)) then
    ...
    barra_componentes.children.data = [Po_barra; Pf_barra];
    
    drawnow(); // Desenha a nova tela

    //Salvando a imagem em gif
    if save_frames then
        format(6); //Até 4 algarismos significativos para salvar o nome do arquivo
        xs2gif(1,'EP3_' + string(i) + '.gif');
    else
